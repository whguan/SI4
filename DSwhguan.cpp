/***************************************************************** 
*COMP 550 SI4- Disjoint Segment

Input : A set of supposedly non-touching blue Segments
Output: VERIFIED iff Segments are non-touching

Date: Sep 14th, 14 by  Wenhua Guan
	...
	for(int k = 0; k < runtimes; k++){
	    ...
	    for(vector<int>::size_type i =0; i < FlagList.size(); i++){
			...
			if(FlagList[i]->typetag == 1){
				//Segment *seg= new Segment(FlagList[i]->Seg->start,FlagList[i]->Seg->end);
				...
				if(Segment::SEGEMENT_INTERSECT(*Below, FlagList[i]->Seg)||
				   Segment::SEGEMENT_INTERSECT(*Above, FlagList[i]->Seg)){
			...
			if(FlagList[i]->typetag==0){
				...
				if(Segment::SEGEMENT_INTERSECT(*Below, *Above)) {
...
/* 	for(vector<int>::size_type i=0; i < RedSeg.size(); i++){ 
 	...
 	for(vector<int>::size_type j=0; j < BlueSeg.size(); j++){
         	if(BlueSeg[j]->start)	delete BlueSeg[j]->start;
		if(BlueSeg[j]->end)     delete BlueSeg[j]->end;
		if(BlueSeg[j])		delete BlueSeg[j];
	} 
	for(vector<int>::size_type i =0; i < FlagList.size(); i++){
//		if(FlagList[i]->P)   delete FlagList[i]->P;
//		if(FlagList[i]->Seg) delete FlagList[i]->Seg;
		if(FlagList[i])	     delete FlagList[i];
	} 
